"""Gen I (Red/Blue/Yellow/Green, JP included) - character maps, scanners, decoders."""
import struct


# ============================================================
# # Gen I EN charmap
# server.py lines 1011-1026
# ============================================================
...
# ============================================================
# # Gen I JP charmap + _KATAKANA_JP
# server.py lines 1031-1073
# ============================================================
...
_GEN1_CHARMAP_JP: dict = {0x7F: ' '}
# Main katakana block 0x80-0xAF (ヘ absent — lives at 0xCD; リ via 0xD8 normalization)
_KATAKANA_JP = 'アイウエオカキクケコサシスセソタチツテトナニヌネノハヒフホマミムメモヤユヨラルレロワヲンッャュョ'
for _i, _c in enumerate(_KATAKANA_JP):
    _GEN1_CHARMAP_JP[0x80 + _i] = _c
_GEN1_CHARMAP_JP.update({
    # Voiced katakana
...
def _gen1_jp_normalize(s: str) -> str:
    return ''.join(_GEN1_JP_H2K.get(c, c) for c in s)

# ============================================================
# # _GEN1_JP_DISASM, _GEN1_JP_REF, _GEN1_JP_NORM_TO_ORIG
# server.py lines 1095-1122
# ============================================================
...
_GEN1_JP_REF = sorted({_gen1_jp_normalize(v) for v in _GEN1_JP_DISASM}, key=len, reverse=True)
_GEN1_JP_NORM_TO_ORIG = {_gen1_jp_normalize(v): v for v in _GEN1_JP_DISASM}

# ============================================================
# # _GEN1_EOS
# server.py lines 1126-1126
# ============================================================
...
# ============================================================
# # _JP_SPECIES_GEN1
# server.py lines 1128-1150
# ============================================================

# Gen I JP species names for splitting merged ROM entries
_JP_SPECIES_GEN1 = {'フシギダネ','フシギソウ','フシギバナ','ヒトカゲ','リザード','リザードン',
...
# ============================================================
# # _scan_gen1_trainer_classes_jp
# server.py lines 1330-1376
# ============================================================
    ...
    while i < len(rom_data) and len(names) < 64:
        ...
        while i < len(rom_data) and rom_data[i] != eos:
            ...
            if ch:
                ...
                pass  # control code
            i += 1
        name = _gen1_jp_normalize(''.join(chars)).strip()
...
# ============================================================
# # _scan_gen1_trainer_classes_en
# server.py lines 1379-1420
# ============================================================
...
# ============================================================
# # _scan_gen1_species_varlen
# server.py lines 1584-1654
# ============================================================
...
def _scan_gen1_species_varlen(rom_data: bytes, charmap: dict, eos: int,
    ...
    disassembly-verified reference (_GEN1_JP_DISASM).
    """
    ...
    all_species = []
    for group in raw:
        text = _gen1_jp_normalize(group)
        while text:
            found = False
            for name in _GEN1_JP_REF:
                if text.startswith(name):
                    all_species.append(_GEN1_JP_NORM_TO_ORIG[name])
                    text = text[len(name):]
                    found = True
                    break
            if not found:
...
# server.py lines 2470-2534
# ============================================================

def decode_gen1_encounters(map_idx: int, current_rom: dict, text_tables: dict) -> str:
...
_GEN1_GROWTH_RATES = {0:'Medium Fast', 1:'Fast', 2:'Slow', 3:'Medium Slow'}

# ============================================================
# # _gen1_resolve_const
# server.py lines 4426-4445
# ============================================================
...
def _gen1_resolve_const(key: str, g1off: dict, current_rom: dict, text_tables: dict) -> int:
    ...
    for i, n in enumerate(sp_list):
        if n and (n.strip() == key.strip() or
                  _gen1_jp_normalize(n).strip() == _gen1_jp_normalize(key).strip()):
            return i
    return -1

# ============================================================
# # _extract_gen12_evo_learnset
# server.py lines 4448-4483
# ============================================================
...
# ============================================================
# # decode_gen12_trainer_class
# server.py lines 4486-4598
# ============================================================

def decode_gen12_trainer_class(class_id, current_rom: dict, text_tables: dict, direct_off: int = None, label: str = None) -> str:
